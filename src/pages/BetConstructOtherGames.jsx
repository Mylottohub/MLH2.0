...
const BetConstructOtherGames = () => {
  ...
  // Measure real window height and calculate iframe scale
  const updateModalHeight = useCallback(() => {
    const height = window.innerHeight;
    setModalHeight(height);
    
    // Calculate scale to fill the entire iframe area
  ...
  const fetchGames = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setGames(list);
        // Filter games by "Others" type only
        const otherGames = list.filter((game) => {
          const gameType = game?.type || game?.gameType || game?.category || "";
          return gameType === "Others";
        });
        setFilteredGames(otherGames);
      } else {
  ...
  // Check authentication before opening game
